...
static void get_operand_data_str(x86_op_t *op, char *str, int len) {
    ...
    if (op->flags & op_signed) {
        switch (op->datatype) {
            ...
            case op_word:
                snprintf(str, len, "%"
                PRId16, op->data.sword );
                return;
            case op_qword:
                snprintf(str, len, "%"
                PRId64, op->data.sqword );
                return;
            default:
                snprintf(str, len, "%"
                PRId32, op->data.sdword );
                return;
        }
    }
    ...
    switch (op->datatype) {
        case op_byte:
            snprintf(str, len, "0x%02"
            PRIX8, op->data.byte );
            return;
        case op_word:
            snprintf(str, len, "0x%04"
            PRIX16, op->data.word );
            return;
        case op_qword:
            snprintf(str, len, "0x%08"
            PRIX64, op->data.sqword );
            return;
        default:
            snprintf(str, len, "0x%08"
            PRIX32, op->data.dword );
            return;
    }
}
