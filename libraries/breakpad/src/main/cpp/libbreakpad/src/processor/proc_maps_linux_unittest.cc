...
for (
size_t i = 0;
i < sizeof(kTestCases) / sizeof(kTestCases[0]); ++i) {
std::vector <google_breakpad::MappedMemoryRegion> regions;
EXPECT_TRUE(ParseProcMaps(kTestCases[i].input, &regions)
);
EXPECT_EQ(1u, regions.
...
continue;
EXPECT_EQ(kTestCases[i]
.permissions, regions[0].permissions);
}
}
